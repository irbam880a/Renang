...
class SettingsDialog(tk.Toplevel):
    def __init__(self, parent, current: dict, on_save):
        ...
        ttk.Label(frm, text="Jumlah Lintasan (1–16):").grid(row=0, column=0, **pad, sticky="w")
        ...
        frm2.grid(row=1, column=0, sticky="ew", padx=12, pady=4)

        ttk.Label(frm2, text="Port:").grid(row=0, column=0, **pad, sticky="w")
        ...
        ttk.Button(frm2, text="↺ Refresh", command=self._refresh_ports).grid(row=0, column=2, **pad)

        ttk.Label(frm2, text="Baud Rate:").grid(row=1, column=0, **pad, sticky="w")
        ...
        ttk.Button(btn_frm, text="Simpan", command=self._save).pack(side="left", padx=6)
        ttk.Button(btn_frm, text="Batal", command=self.destroy).pack(side="left", padx=6)
